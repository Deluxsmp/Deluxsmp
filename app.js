...
// Icons are configured once in icons.js and reused everywhere.
DATA.icons={...DATA.icons,...(typeof VORTEX_ICONS!=='undefined'?VORTEX_ICONS:{})};
const PATH_MODE_MAP={overall:'overall',vanilla:'vanilla',uhc:'uhc',pot:'pot',nethop:'nethop',smp:'smp',sword:'sword',axe:'axe',mace:'mace'};
const pathMode=(location.pathname.split('/').pop()||'').replace(/\.html$/i,'').toLowerCase();
let active=new URLSearchParams(location.search).get('mode')||PATH_MODE_MAP[pathMode]||'overall';
if(!VORTEX_DATA.modes.some(m=>m.id===active)) active='overall';
...
function iconHTML(m,cls='mode-icon-img'){
 ...
 if(!url) return `<span class="icon-fallback"></span>`;
 return `<img class="${cls}" src="${esc(url)}" alt="${esc(m.n)}" loading="eager" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="icon-fallback" style="display:none"></span>`;
}
...
function isTestedResult(r){return !!r && String(r.tier||'').trim()!==''}
...
function renderModes(){modesEl.innerHTML=DATA.modes.map(m=>`<button class="mode ${m.id===active?'active':''}" data-id="${m.id}"><span class="ico">${iconHTML(m)}</span>${esc(m.n)}</button>`).join('');document.querySelectorAll('.mode').forEach(b=>b.onclick=()=>{active=b.dataset.id;history.replaceState(null,'',active==='overall'?'overall.html':active+'.html');document.querySelectorAll('.mode').forEach(x=>x.classList.toggle('active',x===b));search.value='';render()})}
...
function modalFor(p){
 ...
 const tiers=testedModes.map(id=>{const m=modeById(id),r=result(p,id);if(!isTestedResult(r))return `<div class="modal-tier modal-tier-empty"><div class="modal-tier-icon"></div><b>—</b></div>`;return `<div class="modal-tier"><div class="modal-tier-icon">${iconHTML(m,'mini-icon-img')}</div><b class="${tierClass(r.tier||'')}">${esc(r.tier||'')}</b>${r.points!==null&&r.points!==undefined&&r.points!==''?`<small>${r.points} pts</small>`:''}</div>`}).join('');
...
const activeMode=modeById(active);
if(pageTitle) pageTitle.textContent=active==='overall'?'Overall Rankings':activeMode.n+' Rankings';
if(pageSubtitle) pageSubtitle.textContent=active==='overall'?'All competitive Minecraft PvP rankings':'Competitive '+activeMode.n+' PvP leaderboard';
if(DATA.site?.siteName)document.title=DATA.site.siteName+' — '+(active==='overall'?'Overall':activeMode.n)+' Rankings'; const server=document.querySelector('.server'); if(server&&DATA.site?.serverIp) server.innerHTML='<i></i> '+esc(DATA.site.serverIp); renderModes();render();
